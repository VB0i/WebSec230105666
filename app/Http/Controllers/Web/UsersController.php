...
class UsersController extends Controller {
    ...
    public function list(Request $request) {
        // Check if the authenticated user is an Admin or Employee
        if (!auth()->user()->hasRole('Admin') && !auth()->user()->hasRole('Employee')) {
            abort(403); // Forbidden if they don't have the role
        }
    
        // Query to fetch all users for Admin, or only customers for Employee
        $query = User::query();
        ...
        $users = $query->get(); // Get the filtered list of users
        return view('users.list', compact('users')); // Return the view with the users
    }
    ...
    public function doRegister(Request $request) {
    	...
    	try {
            ...
            $user->assignRole('Employee'); // Assign Employee role to a user (if they should have that role)
